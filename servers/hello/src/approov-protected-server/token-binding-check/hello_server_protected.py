...
def _verifyApproovTokenBinding():
    ...
    if not token_binding_header:
        # You may want to add some logging here.
        log.error('Approov Token Binding: Missing the Approov token binding header...')
        return abort(make_response({}, 401))

    if not 'approov_token_claims' in g:
        # You may want to add some logging here.
        log.error('Approov Token Binding: Missing the Approov token claims...')
        return abort(make_response({}, 401))

    if not 'pay' in g.approov_token_claims:
        # You may want to add some logging here.
        log.error('Approov Token Binding: Missing the `pay` in the Approov token claims...')
        return abort(make_response({}, 401))
    ...
    if g.approov_token_claims['pay'] != token_binding_header_encoded:
        log.error('Approov Token binding check failed...')
        return abort(make_response({}, 401))
