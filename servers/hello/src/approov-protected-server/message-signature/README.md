...
This Approov integration example is from where the code example for the [Approov token check quickstart](/docs/APPROOV_TOKEN_QUICKSTART.md) is extracted, and you can use it as a playground to better understand how simple and easy it is to implement [Approov](https://approov.io) in a Python Flask API server.
...
First, you need to install the dependencies. From the `/servers/hello/src/approov-protected-server/message-signature/token-check` folder execute:
...
Now, you can run this example from the `/servers/hello/src/approov-protected-server/message-signature` folder with:
...
Windows (PowerShell)
Open PowerShell in the folder where gen_keys.ps is saved.

powershell
```bash
.\gen_keys.ps
```
powershell -ExecutionPolicy Bypass -File .\Prep-Request.ps
...
```bash
approov token --genExample approov.io
```


```bash
chmod +x prep_request.sh
./prep_request.sh
```
...
```bash
FLASK_APP=hello_server_protected.py flask run --port 8002 --debug
#or
FLASK_APP=hello_server_protected.py flask run --port 8002
```