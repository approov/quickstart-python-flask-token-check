...
Let's start by adding [this method](./server/approov-protected-server.py#L88-L90)
...
Now we need to add [this method](./server/approov-protected-server.py#L92-L108) to
...
Now we need to add [this method](./server/approov-protected-server.py#L110-L122)
...
We also need to add [this method](./server/approov-protected-server.py#L124-L136)
...
Then we need [this method](./server/approov-protected-server.py#L137-L153) to check
...
Finally we need [this method](./server/approov-protected-server.py#L155-L168) to
...
[shapes](./server/approov-protected-server.py#L203-L209) endpoint:
...
the [forms](./server/approov-protected-server.py#L213-L234) endpoint:
