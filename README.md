...
In this example, Approov token check is implemented in `ApproovApplication.extension`. The responsibilities break down as follows:


## Mandatory Approov logic mapping

1. **JWT Approov Token validation (signature + expiry)** is implemented in [`approov()`](./app.py#L135-L197), specifically decode/expiry checks in [`approov()`](./app.py#L152-L168).  
   It uses `jwt.decode(..., algorithms=["HS256"])` and requires `exp`.

2. **Token binding (pay + hash)** is handled in [`approov()`](./app.py#L170-L185), hash helper [`_sha256_b64url_from_str()`](./app.py#L104-L106), and comparator [`_binding_matches()`](./app.py#L109-L112).  
   It computes `base64url(sha256(binding_value))` and compares to `pay`.

3. **Middleware enforcement** is done in [`approov_middleware_enforcement()`](./app.py#L300-L330).  
   Missing/invalid token or binding returns `401 Unauthorized`.

4. **Binding value selection (what gets hashed)** is in [`_build_token_binding_string()`](./app.py#L88-L101).  
   It takes an ordered `bound_headers` list and concatenates header values in that exact order.

5. **Protected route requirements** are defined in [`PROTECTED_ROUTES`](./app.py#L28-L32) and required-header computation in [`_required_headers_for_request()`](./app.py#L129-L132).

6. **Protected routes are registered** in route definitions in [`create_app()`](./app.py#L337-L396), while enforcement remains middleware-driven.
